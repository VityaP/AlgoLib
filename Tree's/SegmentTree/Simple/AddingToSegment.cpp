...
которая обрабатывает два вида запросов:прибавить к отрезку от l до r−1 число v, 
узнать текущее значение в ячейке i. 
Возможно вы слышали, что подобные запросы можно делать с помощью дерева отрезков 
с массовыми операциями (о нем мы поговорим в следующем занятии), 
но конкретно эту задачу можно решить с помощью обычного дерева отрезков.
