...
struct MySegmentTree{
    ...
    void ApplyMaxToSegment(long long l, long long r, long long value, long long x, long long lx, long long rx){
        if( l >= rx || lx >= r){
            return;
        }
        if( lx >= l && rx <= r){
        ...
        long long mid = (lx + rx)/2;
        ApplyMaxToSegment(l, r, value, 2*x + 1, lx, mid);
        ApplyMaxToSegment(l, r, value, 2*x + 2, mid, rx);
        // tree[x] = max(tree[x], tree[2*x + 1]);
        // tree[x] = max(tree[x], tree[2*x + 2]);
    }

    void ApplyMaxToSegment(long long l, long long r, long long value){
        //!!!!!!!!!!!!!!!!!!!!!
        //ATENTION VICTOR [l,r)
        //!!!!!!!!!!!!!!!!!!!!!
        ApplyMaxToSegment(l, r, value, 0, 0, sizeTree);
    }
...
которая обрабатывает два вида запросов:
    ...
    для всех l до r−1 сделать операцию ai=max(ai,v),
...
Далее следует описание операций. Описание каждой операции имеет следущий вид:
    ...
    1 l r v — для всех l до r−1 сделать операцию ai=max(ai,v) (0≤l<r≤n, 0≤v≤109).
...
int main() {
    ...
    for(long long i = 0; i < m; ++i){
        ...
        if( operation == 1){
            long long l, r, value;
            cin >> l >> r >> value;
            tree.ApplyMaxToSegment(l, r, value);
        }
        if( operation == 2){
            long long idx;
            cin >> idx;
