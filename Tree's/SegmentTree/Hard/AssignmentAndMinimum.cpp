...
struct MySegmentTree{
    ...
    long long NEUTRAL_ELEMENT = LLONG_MAX;
    ...
    //         tree[x] = {op_sum(tree[2*x + 1].minMy, tree[2*x + 2].minMy), NO_OPERATION};
    //     }
    // }
    ...
    void push(int x, int lx, int rx){
        ...
        tree[2*x + 1].minMy = op_modify(tree[2*x + 1].minMy, tree[x].set, mid - lx);
        tree[2*x + 2].set = op_modify(tree[2*x + 2].set, tree[x].set, 1);
        tree[2*x + 2].minMy = op_modify(tree[2*x + 2].minMy, tree[x].set, rx - mid);
    ...
    void AssignOnSegment(long long l, long long r, long long value, long long x, long long lx, long long rx){
        ...
        if( lx >= l && rx <= r){
            ...
            tree[x].minMy = op_modify(tree[x].minMy, value, rx - lx);
            return;
        }
        ...
        tree[x].minMy = op_sum(tree[2*x + 1].minMy, tree[2*x + 2].minMy);
    }
